 ...
 * @returns {ProtocolExtract} Extracted information.
 * @private
 */
...
function Url(address, location, parser) {
	...
	// parser.
	//
	if (parser) { url.search = parser(url.search); }
...
function toString(stringify) {
	...
	search = typeof url.search === 'object' ? stringify(url.search) : url.search;
	if (search) { result += search.charAt(0) !== '?' ? `?${search}` : search; }
...
const URLSearchParams = require('./URLSearchParams2');
Object.defineProperty(global.URL.prototype, 'searchParams', {
	  get (context) {
		 const searchParams = new URLSearchParams(this.search);
		 searchParams.set = (key, value) => {
